...
class ImportScripts::Base
  ...
  def create_user(opts, import_id)
    ...
    existing = find_existing_user_by_username(opts[:username])
    if existing && (merge || existing.custom_fields["import_id"].to_s == import_id.to_s)
        puts "User '#{opts[:username]}' already exists in Discourse"
        return existing
    end

    # If username was already encountered in database, chances are that any subsequent matches of same email are inactive double nicks. Migrate users to preserve posting history, but with a bogus email
    double_nick = find_existing_user_by_email(opts[:email])
    if double_nick
        puts "Email '#{opts[:email]}' already exists in database. Migrating user '#{opts[:username]}' with fake email"
    ...
    if u.custom_fields['import_email']
      ...
      user_option.save!
      if u.save
          print "Failed validation for user %s , suspending"  % u.custom_fields['import_email']
  ...
  def find_existing_user_by_username(username)
